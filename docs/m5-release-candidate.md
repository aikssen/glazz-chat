...
Phase 9 and all local M5 acceptance gates completed on 2026-07-24. M5 is ready for
its release commit and GitHub CI, but is not tagged or published.
...
| M5-A05 | QA-005        | Complete visual regression matrix                        | Accepted | 20 reviewed Linux baselines across five states and four viewports; repeat run has zero diff           |
...
1. Commit the completed Phase 9 change set when requested.
2. Push and require presubmit, E2E, and security jobs to pass.
3. Create and push annotated `v0.5.0`.
4. Start Phase 10 only after the published M5 gate.
